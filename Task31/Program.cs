...
//         положительных чисел равна 29, а сумма отрицательных равна -20.
...
int GetSumNegativeElem (int[] arr)
{
    ...
    for (int i = 0; i < arr.Length; i++)
    {
        if(arr[i] < 0) sum += arr[i]; // sumNegative = sumNegative + arr[i];
    }
    return sum;
}

int GetSunPositiveElem (int[] arr)
{
    int sum = 0;
    for (int i = 0; i < arr.Length; i++)
    {
...
int sumPositive = GetSunPositiveElem(array);
